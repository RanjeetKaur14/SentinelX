...
class Track:
    ...
    def predicted_centroid(self) -> Point:
        ...
        forward by that many frames' worth of velocity, or the
        prediction lags further and further behind a fast-moving
        track the longer it's occluded -- which defeats the purpose
        of prediction for exactly the scenario it exists to help
        (a person briefly occluded while moving fast through a dense
        crowd). Falls back to the current centroid when velocity is
