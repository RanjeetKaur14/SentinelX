...
class TrackerConfig:
    ...
    def __post_init__(self):
        ...
        if self.max_disappeared_frames < 0:
            raise ValueError("max_disappeared_frames must be >= 0.")
...
class AnalyticsBufferConfig:
    ...
    def __post_init__(self):
        if self.buffer_length <= 0:
            raise ValueError(
                "buffer_length must be > 0 -- 0 doesn't crash "
                "(deque(maxlen=0) is valid Python) but silently "
                "disables occupancy_growth/zone_growth entirely, "
                "which is a confusing failure mode to debug."
            )
...
class CrowdMetricsConfig:
    ...
    def __post_init__(self):
        if self.growth_window_frames <= 0:
            raise ValueError(
                "growth_window_frames must be > 0 -- a value of 0 "
                "makes the buffer-lookback index equal len(buffer), "
                "which is out of range and raises IndexError on the "
                "very first frame with a non-empty buffer."
            )
